...
def epsilon_greedy(state_vector, epsilon):
    ...
    action_best = np.argmax(q_values_action).numpy()
    object_best = np.argmax(q_values_object).numpy()
...
def deep_q_learning(current_state_vector, action_index, object_index, reward,
    ...
    # TODO Your code here
    q_values_action_curr, q_values_object_curr = model(current_state_vector)

    current = 1/2*(q_values_action_curr[action_index] + q_values_object_curr[object_index])
    target = reward + GAMMA*maxq_next*(not terminal)
    
#    loss = F.mse_loss(q_curr.requires_grad_(True), y.requires_grad_(True))
...
#    epi_reward = None
    ...
    while not terminal:
        ...
        (next_room_desc, next_quest_desc, reward, terminal) = framework.step_game(current_room_desc, 
            current_quest_desc, action_index, object_index)
