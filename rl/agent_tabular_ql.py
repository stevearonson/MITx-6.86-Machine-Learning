...
def epsilon_greedy(state_1, state_2, q_func, epsilon):
    ...
    """
    action_random = np.random.randint(NUM_ACTIONS)
    object_random = np.random.randint(NUM_OBJECTS)
    
    (action_best, object_best) = np.unravel_index(np.argmax(q_func[state_1,state_2,:,:]), (NUM_ACTIONS, NUM_OBJECTS))

    a = np.random.binomial(1, epsilon)
    action_index = a*action_random + (1-a)*action_best
    object_index = a*object_random + (1-a)*object_best
...
def tabular_q_learning(q_func, current_state_1, current_state_2, action_index,
    ...
    Returns:
        None
    """
    q_curr = q_func[current_state_1, current_state_2, action_index, object_index]
    q_next = np.max(q_func[next_state_1, next_state_2, :, :])

    if terminal:
        q_func[current_state_1, current_state_2, action_index, object_index] = (1 - ALPHA)*q_curr + ALPHA*reward
    else:
        q_func[current_state_1, current_state_2, action_index, object_index] = (1 - ALPHA)*q_curr + ALPHA*(reward + GAMMA*q_next)
...
#    for TRAINING_EP in [0.00001, 0.001, 0.1, 0.5, 0.99]:
    for TRAINING_EP in [0.1]:
        ...
        summ_data.append(pd.DataFrame({
                'TRAINING_EP':TRAINING_EP*np.ones(NUM_EPOCHS),
#                'ALPHA':ALPHA*np.ones(NUM_EPOCHS),
                'Epochs':np.arange(NUM_EPOCHS),
                'Reward':np.mean(epoch_rewards_test, axis=0)}))
    ...
    pd.concat(summ_data).set_index(['Epochs', 'TRAINING_EP']).unstack().plot()