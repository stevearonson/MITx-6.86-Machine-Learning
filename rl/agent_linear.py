...
def epsilon_greedy(state_vector, theta, epsilon):
    ...
    (action_best, object_best) = index2tuple(np.argmax(q_value))
...
def linear_q_learning(theta, current_state_vector, action_index, object_index,
    ...
    """
    # TODO Your code here
    q_curr = (theta @ current_state_vector)[tuple2index(action_index, object_index)]
    if terminal:
        q_next = 0
    else:
        q_next = np.max(theta @ next_state_vector)

    theta[tuple2index(action_index, object_index)] += ALPHA*(reward + GAMMA*q_next - q_curr)*current_state_vector
